...
from model.self_Net import CCMNet
from model.FSSNet import FSSNet
from model.PFENet import PFENet
# from model.SCL_1 import SCL_1
# from model.SCL_5 import SCL_5
from model.NERTNet import NTRENet
from model.SDAANet import SDAANet
from model.PFENet_QGNet import PFENet_G
from model.baseline import Baseline
from model.CPANet import cpanet
from model.M3Net import M3Net
from model.new_Net import CFNet
# from model.mpinet import MPINet
# from model.new_Net1 import CFNet1
# from model.new_Net2 import CFNet2
# from model.new_Net3 import CFNet3
# from model.new_Net4 import CFNet4
# from model.new_Net5 import CFNet5
# from model.new_Net6 import CFNet6
# from model.mianet import MIANet
# from model.SCCAN import SCCAN
from model.BAM import OneModel as BAM
# from model.HDMNet import OneModel as HDMNet
# from model.AMFormer import AMNet
# from model.mianet import MIANet
# from model.DCP import OneModel as DCP
# from model.MSANet import OneModel as MSANet
# from model.SCCANPlus import AENet
...
# save_img_path = './exp/MSD_SCCAN/1'

def get_parser():
    parser = argparse.ArgumentParser(description='Few-Shot Semantic Segmentation')
    parser.add_argument('--config', type=str, default='config/IC-FSS/fold0_resnet50.yaml', help='config file')
...
heatmap_dir = './heatmaps_ic'
os.makedirs(heatmap_dir, exist_ok=True)

def hook_fn(module, input, output):
    # 这里我们仅为每个epoch的第一批数据生成热力图
    for i, feature_map in enumerate(output):
        if i >= 1:  # 限制只生成一个batch中第一个图像的热力图
            break

        # 用于生成热力图的数据
        np_feature_map = feature_map.cpu().data.numpy()

        # 计算要生成热力图的feature map的数量（最多生成前10个）
        num_feature_maps = min(np_feature_map.shape[0], 10)

        for j in range(num_feature_maps):
            plt.matshow(np_feature_map[j], cmap='jet')
            plt.axis('off')
            plt.xticks([])
            plt.savefig(f'{heatmap_dir}/feature_map_{j}.png', bbox_inches='tight', pad_inches = -0.1)
            plt.close()
...
def main_worker(argss):
    ...
    # 初始化模型
    model = CCMNet(layers=50, classes=2, criterion=nn.CrossEntropyLoss(ignore_index=255),
                   pretrained=True, shot=args.shot, ppm_scales=args.ppm_scales, vgg=False)

    # model.tb.register_forward_hook(hook_fn)

    # 处理backbone
    optimizer = backbone_optimizer(model, args)
    # optimizer = model.get_optim(model)
        ...
        if args.evaluate and (epoch % 1 == 0 or (args.epochs<=50 and epoch%1==0)):
            ...
            if class_miou > max_iou:
                ...
                if os.path.exists(filename):
                    os.remove(filename)            
                filename = args.save_path + '/train_epoch_' + str(epoch) + '_'+str(max_iou)+ '_'+str(mIoU_val)+'.pth'
...
def validate(val_loader, model, criterion):
    ...
    for e in range(10):
        for i, (input, target, nomimg, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            # 热力图
            # model.Tripletencoder.register_forward_hook(hook_fn)
            # model.CS.register_forward_hook(hook_fn)
            # model.MF.register_forward_hook(hook_fn)
            # model.Subnet.register_forward_hook(hook_fn)
            ...
            # if sum(intersection.float() / union.float()) / 2 >= 0.90:
            # q_pred = output[:, :ori_label.size(1), :ori_label.size(2)]
            # q_pred = torch.squeeze(q_pred).cpu().numpy()
            # cv2.imwrite(os.path.join(save_img_path, str(args.split) + '_' + str(i) + '_' + str(e) + 'query_pred.png'), 255 * q_pred.astype(np.uint8))
